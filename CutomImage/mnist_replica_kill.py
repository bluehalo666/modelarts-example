...
flags = tf.app.flags
flags.DEFINE_string("data_url", "/tmp/mnist-data",
...
def create_done_queue(index, num_worker):
  with tf.device("/job:ps/task:%d" % (index)):
    return tf.FIFOQueue(num_worker, tf.int32, shared_name="done_queue" + str(index))
...
def main(unused_argv):
  ...
  if not FLAGS.existing_servers:
    ...
    if FLAGS.job_name == "ps":
      queue = create_done_queue(FLAGS.task_index, len(worker_spec))
  ...
  with tf.device(
    ...
    finalize_ops = [q.enqueue(1) for q in create_done_queues(len(ps_spec), len(worker_spec))]
